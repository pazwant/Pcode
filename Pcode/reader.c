...
#include "header2.h"
#include <math.h>
void reader (char* fname, int stek) {
    int line_num_ = 0;
    int z = 0;
    int v = 0;
    int stek_n3 = 0;
    ...
    while (fgets(bufy,sizeof(bufy),reader) != NULL) {
        ...
        for(z=0 ; z <= (stek-stek_n3) ; ++z) {
            if(line_num_ == ((stek_n3+z)-2))
                printf ("\e[9C\e[7h\e[96m%s", bufy);
        }
        if(line_num_+1 == stek) {
            
            printf ("  \e[31mLine number %d ===>%s\e[32m", stek, bufy);
            sleep(0.5);
            fseek(reader,start,stek);
            if(strstr(bufy,"}") == NULL) {
                int c;
                do {
                    c = fgetc(reader); printf("\e[7h\e[96m%c", c);
                }
                while(c != '}');
                printf("\e[7h\e[9C\n");}
        }
