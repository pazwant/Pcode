#include "header2.h"
#include "rep.h"
int messagex(char *filename, char *str, char *def, char *rsk)
{
    int code_flow (int , char * , char *, char *, char *, char *);
    char *arr_lines, *line;
    ...
    for (int i = 0; i <= num_lines-1; i++) {
        ...
        while (fgets(aaa.buff,sizeof(aaa.buff),fpc) != NULL) {
            ...
            if  ((strstr(aaa.buff, str) != NULL) && strstr(aaa.buff, "* ") == NULL ) {
                
                fdw = 79;
                
                code_flow(line_num+1,aaa.z,aaa.buff,str, def, rsk); }
